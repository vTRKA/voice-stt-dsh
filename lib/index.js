...
export function apply(ctx) {
  ...
  if (webServer !== undefined) ctx.effect(() => {
    const startRoute = webServer.register({ kind: 'exact', path: START_ROUTE, handler: async (_req, res) => {
      const ok = await start()
      res.writeHead(ok ? 204 : 503)
      res.end()
    } })
    const stopRoute = webServer.register({ kind: 'exact', path: STOP_ROUTE, handler: (_req, res) => {
      stop()
      res.writeHead(204)
      res.end()
    } })
    return () => { startRoute(); stopRoute(); stop() }
  }, 'Parakeet runtime on demand')
}
